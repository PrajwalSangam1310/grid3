...
ros::Subscriber<grid3::botCommand> rosCmdSub("grid3/controller/cmd4", &cmdSubCb);

//ros::Publisher imuPub("grid3/imu", &tempImu);


// Configurations
uint8_t address[6] = "4Node";
...
void radio_init() {
  ...
  if (!radio.begin()) {
    nh.loginfo("[controller 4]radio hardware is not responding!!");
  ...
  radio.stopListening();
  nh.loginfo("[controller 4]Radio: Initiated");
...
bool transmit_data(){
  ...
  if (report) {
    nh.loginfo("[controller 4]Radio: transmission successful");
    return true;
  }
  else {
    nh.loginfo( "[controller 4]Radio: Transmission failed or timed out");      // payload was not delivered
    return false;
  }
}
