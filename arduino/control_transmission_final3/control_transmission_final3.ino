...
ros::Subscriber<grid3::botCommand> rosCmdSub("grid3/controller/cmd3", &cmdSubCb);

//ros::Publisher imuPub("grid3/imu", &tempImu);


// Configurations
uint8_t address[6] = "3Node";
...
void radio_init() {
  ...
  if (!radio.begin()) {
    nh.loginfo("[controller 3]radio hardware is not responding!!");
...
bool transmit_data(){
  ...
  if (report) {
    nh.loginfo("[controller 3]Radio: transmission successful");
    return true;
  }
  else {
    nh.loginfo( "[controlelr 3]Radio: Transmission failed or timed out");      // payload was not delivered
    return false;
  }
}
