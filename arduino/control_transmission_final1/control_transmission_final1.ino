...
ros::Subscriber<grid3::botCommand> rosCmdSub("grid3/controller/cmd1", &cmdSubCb);
...
void radio_init() {
  ...
  if (!radio.begin()) {
    nh.loginfo("[controller 1]radio hardware is not responding!!");
  ...
  radio.stopListening();
  nh.loginfo("[controller 1]Radio: Initiated");
...
bool transmit_data(){
  ...
  if (report) {
    nh.loginfo("[controller 1]Radio: transmission successful");
    return true;
  }
  else {
    nh.loginfo( "[controller 1]Radio: Transmission failed or timed out");      // payload was not delivered
    return false;
  }
}
