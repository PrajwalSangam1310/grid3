...
void setup() {
  ...
  pinMode(buttonListenJoy, INPUT_PULLUP);
  radio_init();
...
void loop() {
  ...
  listenJoy = digitalRead(buttonListenJoy);

  Serial.println(button);
  Serial.println(listenJoy);
  ...
  if (button == 0) {
    ...
    if (listenJoy == 0 and button == 0) {
      instruction = 2;
      Serial.println("Listen to joy");
    }

    else if (listenJoy == 1 and button == 0) {
      instruction = 3;
      Serial.println("Listen to manual");
    }
  }
  else {
    if (mapX < -400) {
      instruction = 0; //backward
      Serial.println("Stopping Command");
    }
    else if (mapX > 400) {
