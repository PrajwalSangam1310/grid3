...
class Brain:
    ...
    def __init__(self, number_of_robots, package_list_0, package_list_1, pickup_points = ((5,15),(10,15))) -> None:
        ...
        self.packageLoaded.status = [True]*4
        ...
        temp =['Mumbai', 'Delhi', 'Kolkata', 'Chennai','Bengaluru','Hyderbad','Pune','Ahemdabad','Jaipur'][::-1]
        self.des_names = ['Closing to induction point','Induction point']+temp
        self.des_names = self.des_names[-1::-1]
        self.waiting_points = [(1,9),(14,9)]
    ...
    def at_waiting_point(self, robot_index):
        robot_location = self.get_current_location(robot_index)
        
        for i in range(len(self.waiting_points)):
            if self.waiting_points[i] == robot_location:
                return True
        return False
    ...
    def next_goal_point(self, i):
        ...
        if self.robots[i].isDropping:
            ...
            if time.time() - self.droppingPeriod > self.droppingStartTimes[i]:
                ...
                self.publishTrajectoryPlanningCommand(i, 11)
            else:
                #dropping in progress
                print(f"[Brain, {i}]: Dropping in progress!")
                return

        elif self.at_waiting_point(i):
            print(f"[Brain, {i}]: At waiting point")
